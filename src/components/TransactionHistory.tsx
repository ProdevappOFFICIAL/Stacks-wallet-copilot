    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] m-4 flex flex-col"
                          ...
                          {tx.type === TransactionType.STX_TRANSFER && (
                            <div className="text-sm text-gray-600 mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span>{(tx as STXTransferTransaction).amount} STX to {abbreviateAddress((tx as STXTransferTransaction).recipientAddress)}</span>
                              </div>
                            </div>
                          )}
                          
                          <div className="text-xs text-gray-500 space-y-1">
                            <div>From: {abbreviateAddress(tx.senderAddress)}</div>
                            <div>{tx.timestamp.toLocaleString()}</div>
                            {tx.memo && <div>Memo: {tx.memo}</div>}
                            {tx.errorMessage && (
                              <div className="text-red-600">Error: {tx.errorMessage}</div>
                            )}
                          </div>
                        </div>
                      </div>
